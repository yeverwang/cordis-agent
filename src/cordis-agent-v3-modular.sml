(* ============================================================
   cordis-agent-v3-modular.sml  --  v3.5 (functor-typed services)
   -----------------------------------------------------------
   The story in one paragraph.

   In v3 every service lived in ONE global (string, exn) map.
   To fit into that map, each service value had to be smuggled
   through a per-service `exception Box of t`. The pattern
   works but is a downcast: the compiler cannot tell you have
   mixed up `Calc` and `Search`, and a mistyped service name
   is a silent no-op. That kind of run-time brittleness is
   exactly what an ML type system exists to eliminate.

   v3-modular replaces the shared exn table with

       functor MakeService (S : SERVICE)

   Each application of the functor manufactures a FRESH module
   with its own private `t option ref` and its own subscriber
   list. `Logger`, `Calc`, `Search` and `Write` are now four
   different modules with four different `t` types. There is
   no more exn boxing, no more string lookup, no more downcast:

       Logger.provide expects  { info   : string -> unit }
       Calc.provide   expects  { invoke : string -> int   }

   Passing one to the other is a COMPILE-TIME type error --
   the same shape of error an OCaml Koishi port would raise.

   Everything else -- scope tree, cascade dispose, fork,
   supervisor, workers, scripts -- is identical to v3. That
   is deliberate: the point is to swap the service layer
   under the same 3-worker demo without changing any observ-
   able behaviour.
   ============================================================ *)
...
(* ============================================================
   2.  THE FUNCTOR  -----------------------------------------
   -----------------------------------------------------------
   Every service is a `MakeService(S)` application.  Its state
   is a *private* `t option ref` and a *private* subscribers
   list.  Nothing crosses module boundaries as `exn`.
   ============================================================ *)
signature SERVICE = sig
  type t
  val name : string   (* purely for logging + dependency reporting *)
end
...
functor MakeService (S : SERVICE) : sig
  ...
  val use       : unit -> t             (* raises when not present *)
  val subscribe : (unit -> unit) -> unit
end = struct
  type t = S.t
  val name  = S.name
  val value : t option ref            = ref NONE
  val subs  : (unit -> unit) list ref = ref []
  fun notify () = List.app (fn f => f ()) (!subs)
  fun provide v =
    let val kind = case !value of NONE => "provide" | _ => "replace"
    in log ("  [SVC] " ^ kind ^ " '" ^ name ^ "'")
     ; value := SOME v
     ; notify ()
     ; fn () =>
         ( log ("  [SVC] retract '" ^ name ^ "'")
         ; value := NONE
         ; notify () )
    end
  fun get () = !value
  fun use () = case !value of
                   SOME v => v
                 | NONE   => raise Fail ("service " ^ name ^ " not present")
  fun subscribe f = subs := f :: !subs
end

(* ------------------------------------------------------------
   3. Dependency descriptor
   -----------------------------------------------------------
   SML has no first-class modules, so we lift the two things a
   plugin needs from a dependency -- "is it present?" and "how
   do I subscribe?" -- into a small first-order record. Every
   `MakeService` / `MakeTool` module knows how to build one.
   The point: dependency edges no longer travel by *string*.
   ------------------------------------------------------------ *)
type readiness = {
  name      : string,
  present   : unit -> bool,
...
fun dep (name : string,
         get  : unit -> 'a option,
         subscribe : (unit -> unit) -> unit) : readiness =
  { name      = name,
    present   = fn () => Option.isSome (get ()),
    subscribe = subscribe }

(* ------------------------------------------------------------
   4. Logger service, typed
   ------------------------------------------------------------ *)
structure LoggerSpec = struct
  type t = { info : string -> unit }
  val  name = "logger"
end
structure Logger = MakeService(LoggerSpec)
val loggerDep : readiness = dep (Logger.name, Logger.get, Logger.subscribe)

(* ------------------------------------------------------------
   5. Tool functor (built on MakeService)
   -----------------------------------------------------------
   TOOL_SPEC keeps v3's string-in / string-out helpers so the
   ReAct agent's CallTool byte-code lookup still works, but
   the invoker itself is fully typed: `Calc.provide` takes
   `{ invoke : string -> int }` and nothing else.
   ------------------------------------------------------------ *)
...
functor MakeTool (S : TOOL_SPEC) : sig
  type input  = S.input
  ...
  val subscribe : (unit -> unit) -> unit
  val invokeStr : string -> string
end = struct
  type input  = S.input
  type output = S.output
  type impl   = { invoke : input -> output }
  structure Slot = MakeService(struct
    type t   = impl
    val name = "tool/" ^ S.name ^ "@" ^ S.version
  end)
  val name      = Slot.name
  val provide   = Slot.provide
  val get       = Slot.get
  val subscribe = Slot.subscribe
  fun invokeStr arg =
    case Slot.get () of
        NONE => "error: tool " ^ name ^ " not loaded"
      | SOME {invoke} =>
...
structure Calc = MakeTool(CalcSpec)
val calcDep : readiness = dep (Calc.name, Calc.get, Calc.subscribe)
...
structure Search = MakeTool(SearchSpec)
val searchDep : readiness = dep (Search.name, Search.get, Search.subscribe)
...
structure Write = MakeTool(WriteSpec)
val writeDep : readiness = dep (Write.name, Write.get, Write.subscribe)

(* ------------------------------------------------------------
   6. ctx + events (unchanged shape from v3)
   ------------------------------------------------------------ *)
...
(* ------------------------------------------------------------
   7. Plugin driver
   -----------------------------------------------------------
   Only difference vs v3:  `inject : readiness list`  (typed)
                    was:  `inject : string list`      (untyped)
   ------------------------------------------------------------ *)
type 'cfg plugin = {
  name   : string,
  inject : readiness list,
  apply  : ctx -> 'cfg -> unit
}
...
fun plugin (parent as Ctx {scope=pscope, events=_}) (p : 'cfg plugin) (cfg : 'cfg) =
  let val childScope = ref (newScope (#name p) (SOME pscope))
      ...
      fun ready ()   = List.all (fn r => #present r ()) (#inject p)
      fun missing () = List.map #name
                         (List.filter (fn r => not (#present r ())) (#inject p))
      fun run () =
      ...
      fun reload () =
        ...
        ; running := false; run () )
  in List.app (fn r => #subscribe r reload) (#inject p); run ();
     (fn () => disposeScope (!childScope)) end

(* ------------------------------------------------------------
   8. Provider plugins (typed calls into their service modules)
   -----------------------------------------------------------
   `Logger.use ()` returns { info : string -> unit } at compile
   time. Replace `Logger` with `Calc` and the compiler catches
   it before you can save the file.
   ------------------------------------------------------------ *)
val loggerPlugin : unit plugin = {
  name   = "logger",
  inject = [],
  apply  = fn ctx => fn () =>
    let val stop = Logger.provide
                     { info = fn s => log ("      [LOG] " ^ s) }
...
(* ------------------------------------------------------------
   10. Agent-level tool registry
   -----------------------------------------------------------
   ReAct uses `LoadTool "calc"` -- a *runtime* string name, at
   the AGENT layer. That is genuinely dynamic (the LLM chooses
   at run time), so string dispatch is fine here. What we
   removed is string dispatch INSIDE the service layer.
   ------------------------------------------------------------ *)
type toolEntry = {
  mount     : ctx -> unit,
...
(* ------------------------------------------------------------
   10b. LLM driver — real endpoint via subprocess
   -----------------------------------------------------------
...
fun llmNext (base : string) (token : string) (model : string)
  ...
  let
    (* Unique temp file names for this process, so concurrent calls
       ...
       `tick` is the shared log counter; a private counter avoids
       depending on the log's cadence for file-name uniqueness. *)
    val llmCounter = ref 0
    val llmStamp =
      let val c = !llmCounter
      in llmCounter := c + 1; c end
    val pid      = Posix.ProcEnv.getpid ()
    val stamp    = Int.toString (SysWord.toInt (Posix.Process.pidToWord pid))
                 ^ "-" ^ Int.toString llmStamp
...
(* ------------------------------------------------------------
   11. Fork + supervisor state (unchanged)
   ------------------------------------------------------------ *)
type childHandle = {
  id    : string,
  ctx   : ctx,
...
type supervisorState = { children : childHandle list ref }

(* ------------------------------------------------------------
   12. Reasoner (unchanged from v3)
   ------------------------------------------------------------ *)
...
(* ------------------------------------------------------------
   13b. Select the `next` used by the top supervisor.
   -----------------------------------------------------------
   Default (CORDIS_LLM_REAL unset): scriptedNext, the original demo.
   When CORDIS_LLM_REAL=1 AND both a token (ANTHROPIC_AUTH_TOKEN / AUTH_TOPIC)
   + ANTHROPIC_BASE_URL are set: llmNext reads them from the environment
   (never hardcoded) and drives the supervisor through the real gateway.
   ============================================================ *)
...
fun main () =
  let
    val () = section "PHASE 0 - TYPE LEDGER (compile-time separation)"
    val _ = log "  Logger.t : { info   : string -> unit }"
    val _ = log "  Calc.t   : { invoke : string -> int    }"
    val _ = log "  Search.t : { invoke : string -> string }"
    val _ = log "  Write.t  : { invoke : string -> string }"
    val _ = log "  four functor applications -> four private slots"
    val _ = log "  Calc.provide(Logger.use()) is a COMPILE-TIME type error"

    val () = section "PHASE 1 - root: install logger (via typed module)"
    val root = rootCtx ()
    val _ = plugin root loggerPlugin ()
    val _ = case Logger.get () of
              SOME _ => log "  Logger.get () returned SOME { info = <fn> }"
            | NONE   => log "  Logger.get () returned NONE (should not happen)"

    val () = section "PHASE 2 - launch Supervisor"
    ...
    val _ = log ("  supervisor has "
                 ^ Int.toString (length (!(#children supState)))
                 ^ " tracked children:")
    val _ = List.app (fn c =>
              log ("    - " ^ #id c ^ " alive="
                   ^ Bool.toString (#alive c ())
                   ^ " scope=<" ^ scopeName (ctxScope (#ctx c)) ^ ">"))
              (!(#children supState))

    val () = section "PHASE 4 - artifacts so far"
    val _ = listArtifacts ()

    val () = section "PHASE 5 - PARTIAL DISPOSE: kill only 'slow'"
    ...
    val () = section "PHASE 8 - verify: each typed slot is empty"
    val _ = (case Logger.get () of SOME _ => log "  [+] Logger still live"
                                 | NONE   => log "  [-] Logger gone")
    val _ = (case Calc.get () of SOME _ => log "  [+] Calc still live"
                               | NONE   => log "  [-] Calc gone")
    val _ = (case Search.get () of SOME _ => log "  [+] Search still live"
                                 | NONE   => log "  [-] Search gone")
    val _ = (case Write.get () of SOME _ => log "  [+] Write still live"
                                | NONE   => log "  [-] Write gone")

    val () = section "PHASE 9 - dispose root"
